...
class GetNearCinemaForMovie(private val clientRepository: ClientRepository) {
    ...
    suspend operator fun invoke(lat: Double, lng: Double, distance: Double, movieId: Int): List<Cinema> {
        return clientRepository.getNearCinemasForMovie(lat, lng, distance, movieId)
    }
}